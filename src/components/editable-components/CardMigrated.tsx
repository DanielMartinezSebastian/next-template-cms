...
const CardSchema = z.object({
  ...
  description: z.string().optional().default('This is a sample card description that shows how content will be displayed in the card component.'),
  image: z.string().url('Must be a valid URL').optional().default('https://images.placeholders.dev/400x250?text=Card%20Image&bgColor=%236b7280&textColor=%23ffffff'),
  imageAlt: z.string().optional().default('Card image'),
  buttonText: z.string().optional().default('Learn More'),
  buttonLink: z.string().url('Must be a valid URL').optional().default('#'),
  variant: z.enum(['default', 'outlined', 'elevated']).default('default'),
  imagePosition: z.enum(['top', 'left', 'right']).default('top'),
  className: z.string().optional()
});

export interface CardProps extends z.infer<typeof CardSchema> {}
...
export default withEditable(CardComponent, {
  metadata: {
    category: 'content',
